...
class H36M(data.Dataset):
  def __init__(self, opt, split):
    ...
    annot = {}
    tags = ['action', 'bbox', 'camera', 'id', 'joint_2d',
            'joint_3d_mono', 'subaction', 'subject', 'istrain']
    f = File('../data/h36m/annotSampleTest.h5', 'r')
    for tag in tags:
      annot[tag] = np.asarray(f[tag]).copy()
    f.close()

    ids = np.arange(annot['id'].shape[0])[annot['istrain']
  ...
  def LoadImage(self, index):
    ...
    path = '{}/{}/{}_{:06d}.jpg'.format(ref.h36mImgDir,
                                        folder, folder, self.annot['id'][index])
    #print 'path', path
    img = cv2.imread(path)
    return img
