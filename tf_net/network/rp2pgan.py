...
class rP2PGAN(object):
    def __init__(self, mode='train', sample_dir='samples', checkpoint_dir="./checkpoint",
                 epoch=30000, aspect_ratio=1, batch_size=64, ngf=64, ndf=64, scale_size=286, train_size=4000, test_size=1000, flip=True, label_dim=15, learning_rate=0.0002,
                 beta1=0.5, l1_weight=100, gan_weight=1, dataset_name='H36M', input_width=128, input_height=128):
        self.sample_dir = os.path.join(
            globalConfig.p2pr_pretrain_path, sample_dir)
        self.epoch = epoch
        ...
        self.checkpoint_dir = os.path.join(
            globalConfig.p2pr_pretrain_path, checkpoint_dir)
    ...
    def build_model(self):
        with tf.variable_scope("p2p") as scope:
            ...
            self.D = self.build_discriminator(
                self.image_input, gray2rgb(self.image_target), reuse=False)
            #fake D
            self.D_ = self.build_discriminator(
                self.image_input, gray2rgb(self.G), reuse=True)
            ...
            self.g_loss_L1 = tf.reduce_mean(
                tf.abs(self.image_target - self.G))
    ...
    def predict(self):
        ...
        batch_idxs = int(2168/self.batch_size)
        with tf.Session() as self.sess:
            ...
            for idx in xrange(0, int(batch_idxs)):
                batch_labels = np.zeros((self.batch_size, self.label_dim))
                batch_input = img[idx *
                ...
                for i in range(samples.shape[0]):
                    im = 255*(samples[i]+1)/2
                    cv2.imwrite(self.sample_dir+"/" +
                                "{:04d}".format(idx*self.batch_size+i)+".jpg", im)
    ...
    def test(self, valid_dataset):
        ...
        if not os.path.exists(self.sample_dir):
            os.makedirs(self.sample_dir)
        test_labels, _, test_img, test_img_rgb, _, _ = prep_data(
            valid_dataset, self.batch_size, skel=False)
        im = cv2.imread('./men-836175_960_720.jpg')
        im_gr = cv2.cvtColor(im, cv2.COLOR_RGB2GRAY)

        im = (im-127.5)/127.5
        im_gr = (im_gr-127.5)/127.5
        
        im = cv2.resize(im, (128, 128))
        im_gr = cv2.resize(im_gr, (128, 128))
        im_gr = np.reshape(im_gr, (128, 128, -1))
    ...
    def train(self, train_dataset, valid_dataset):
        ...
        train_labels, _, train_img, train_img_rgb, _, batch_idxs = prep_data(
            train_dataset, self.batch_size,skel=False)
        test_labels, _, test_img, test_img_rgb, _, _ = prep_data(
            valid_dataset, self.batch_size,skel=False)
        ...
        with tf.Session() as self.sess:
            ...
            self.writer = SummaryWriter(
                os.path.join(globalConfig.p2p_pretrain_path, "logs"), graph=self.sess.graph, filename_suffix='.rp2pGAN')
...
if __name__ == '__main__':
    ...
    if globalConfig.dataset == 'H36M':
        ...
        val_ds = Dataset()
        ds = Dataset()
        ds.loadSkate(200, mode='train', tApp=True, replace=False)
        val_ds.loadSkate(64, mode='valid', tApp=True, replace=False)
    else:
