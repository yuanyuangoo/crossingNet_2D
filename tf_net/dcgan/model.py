...
class DCGAN(object):
    ...
    if self.dataset_name == 'mnist':
      ...
      if 'h36m' in self.dataset_name:
        data_path = os.path.join(
            self.data_dir, 'h36m', self.input_fname_pattern)
      else:
        data_path = os.path.join(
  ...
  def train(self, config):
    ...
    if config.dataset == 'mnist' or config.dataset == 'h36m':
    ...
    for epoch in xrange(config.epoch):
      if config.dataset == 'mnist' or config.dataset =='h36m':
        batch_idxs = min(
            len(self.data_X), config.train_size) // config.batch_size
      elif config.dataset=='h36m_nolabel':
        self.data = glob(os.path.join(
            config.data_dir, 'h36m', self.input_fname_pattern))
        np.random.shuffle(self.data)
        batch_idxs = min(
